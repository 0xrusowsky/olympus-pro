...
BOT_TOKEN = os.environ["DISCORD_BOT_TOKEN_ANGLE_EUR"]
...
def get_prices():
    ...
    baseTokenPrice = cg.get_price(ids='tether-eurt', vs_currencies='usd')['tether-eurt']['usd']
    print(baseTokenPrice)
    payoutTokenPrice = cg.get_price(ids='angle-protocol', vs_currencies='usd')['angle-protocol']['usd']

    bondClosed = maxDebtReached(bond_address='0x476aFB11A5105155F6B590d11d865280E3EF02B3')
    bondEmpty, rewardsLeft, rewardsUSDLeft, treasury_address = emptyTreasaury(treasury_address='0x40AdeDc0b097717583eD2C97645eB46e5B58CC78', payout_token_address='0x31429d1856aD1377A8A0079410B297e1a9e214c2', payout_token_price=payoutTokenPrice)

    bondDisc = bond_discount(bond_address='0x476aFB11A5105155F6B590d11d865280E3EF02B3', bondPrice=baseTokenPrice, payoutTokenPrice=payoutTokenPrice, maxReached=bondClosed, noFunds=bondEmpty)
...
async def check_discounts():
    ...
    for guild in client.guilds:
        ...
        try:
            if bondDisc == -999:
                ...
                await client.change_presence(activity=discord.Activity(type=discord.ActivityType.playing, name=f'⛔ agEUR Bonds'))
            elif bondDisc < 0:
    ...
    if rewardsUSDLeft <= 10000 and alert_1 is False:
        ...
        embed.add_field(name='Rewards left in Payout Token', value=f'{rewardsLeft:,.2f} ANGLE', inline=True)
